...
getwd()
library(data.table)
### Proceso para convertir .dat en csv
...
write.csv(venuesData,file="datasets/foursquare/datasets_csv/venues.csv", row.names = F)
...
readLines("datasets/foursquare/umn_foursquare_datasets/users.dat",n=2)
usersData<-read.delim('datasets/foursquare/umn_foursquare_datasets/users.dat', header = F, sep="|",skip=2) 
names(usersData)<-c("id", "latitude", "longitude")
head(usersData)
tail(usersData)
usersData<-usersData[-2153470,]
write.csv(usersData,file="datasets/foursquare/datasets_csv/users.csv", row.names = F)
