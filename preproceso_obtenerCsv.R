...
readLines(getDataSetPath("socialgraph.dat"),n=2)
socialData<-read.delim(getDataSetPath("socialgraph.dat"), header = F, sep="|",skip=2) 
names(socialData)<-c("first_user_id","second_user_id")
head(socialData)
tail(socialData,n=100)
socialData<-socialData[-27098473:-27098488,]
write.csv(socialData,file=getDataSetPath("socialgraph.csv"), row.names = F,quote = F)
